...
- **QR-сканер** — сканирование ссылок и AWG-конфигов
- **QR-обмен** — поделиться сервером, AWG-конфигом или подпиской через QR-код
- **Открытые источники** — 10+ бесплатных подписок (можно полностью отключить/удалить)
...
NurVPN в настоящее время проходит проверку для F-Droid. После одобрения вы найдёте его здесь:
[Страница NurVPN в F-Droid](https://f-droid.org/packages/com.nurvpn.app/)
